...
import (
	"github.com/502388088/px-training-svc/vcap"
...
func SetupServer() {
	...
	router.Methods("POST").Path(base_path  + "people").HandlerFunc(HandleAddProfile) // C
	router.Methods("GET").Path(base_path  + "people").HandlerFunc(HandleGetProfile) // R
	router.Methods("PUT").Path(base_path  + "people").HandlerFunc(HandleUpdateProfile) // U
	router.Methods("DELETE").Path(base_path  + "people").HandlerFunc(HandleDeleteProfile) // D
